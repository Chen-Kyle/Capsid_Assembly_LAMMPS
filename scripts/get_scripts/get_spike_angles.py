...
def print_spike_angle_data(pkl_data, pkl_file_path):
    """
    Prints out the spike angle data to the same directory as the pkl file
    Uses the format:{Frame#},{Interface},{Binding_angle}
    """
    print("Printing Binding Angles...\n")
    out_file_name = pkl_file_path.replace('complete_cluster_data.pkl', 'spike_angles.csv')
    interface_data = pkl_data['interface_data']
    with open(out_file_name, "w") as f:
        # Header
        f.write("Frame#,Interface,Spike Angle\n")

        for frame in interface_data.keys():
            for interface in interface_data[frame].keys():
                value = interface_data[frame][interface]['spike_angle']
...
if __name__ == '__main__':
    ...
    print_spike_angle_data(pkl_data, args.pkl)