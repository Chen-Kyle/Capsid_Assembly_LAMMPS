...
# HBV Decamer CG Oligomer Analysis Batch Cluster Script (local version)
#
...
#
# Same as run_detect_clusters_batch.sh but without the #SBATCH header, so it
# can be run directly on a local machine:
